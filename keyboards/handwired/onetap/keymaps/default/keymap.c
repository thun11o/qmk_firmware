...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [2] = LAYOUT_split_3x6_4(
        _______,    UM_TGL,  UM_LNX,  UM_MAC,     UM_WCP,  XXXXXXX,             KC_HOME, KC_PGDN, KC_PGUP, KC_END,  KC_LWIN, KC_DEL,
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    if(record->event.pressed) {
        if(process_unicode_mode(keycode)) {
            return false;
        }

        if(process_special_char(keycode)) {
            return false;
        }
...
bool process_detected_host_os_user(os_variant_t detected_os) {
    return set_unicode_mode(detected_os);
}
